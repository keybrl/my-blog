...
然后在文章中用 `![hhh]({{ site.oss_url }}/images/hhh.png)` 就可以访问到刚刚那张图片。这样以后要换 bucket ，或者要给这个 bucket 加 cdn ，用自己的域名，都可以很方便修改。
